...
let inputBlock = [0,5,10,0,11,14,13,4,11,8,8,7,1,4,12,11];
let testInputBlock = [0, 2, 7, 0];

let allocator = (input)=>{
  let previousBlocks = new Set();
  let redistributionCount = 0;
  while(!previousBlocks.has(input.toString())){
    redistributionCount++;
    // add current configuration to known blocks
    previousBlocks.add(input.toString());
